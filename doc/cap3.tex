...
associado, mas outras combinações podem ocorrer\footnote{Um exemplo onde pode
ocorrer uma combinação ``um para muitos'' é o caso onde existe uma fonte fraca
...
detectado no \galex. O caso inverso implicaria numa combinação ``muitos para
um''}. O artigo apresenta os resultados para dados do \SDSS DR6 e \galex GR3. Na
tabela \ref{tab:SDSSxGalexMatches} há a quantidade de objetos encontrados nos
{\em surveys} AIS e MIS (estrelas e galáxias) que estão unicamente identificados
com um objeto do \SDSS, utilizando os dados mais recentes ({\em crossmatch}
entre \SDSS DR7 e \galex GR6, disponível no {\em CasJobs} do
\galex\footnote{{\em CasJobs} do \galex:
\url{http://galex.stsci.edu/casjobs/}}). A técnica utilizada por
\citeauthor{Budavari2009} agora faz parte do {\em pipeline} do \galex. A
distribuição do {\em CasJobs} inclui as ferramentas necessárias para fazer o
{\em crossmatch} espacial entre bancos de dados.

% TODO: Preencher tabela de objetos com crossmatch 1x1. 
\begin{table}
	\caption[Número de objetos correlacionados entre \SDSS DR7 e \galex GR6.]
	{Número de objetos do \SDSS DR7 identificados no catálogo do \galex, por survey
	e tipo. Apenas são considerados objetos que têm relação ``um para um'' entre
	\SDSS e \galex.}
	\begin{tabular}{l r r}
		Tipo    &     AIS &     MIS \\ 
		\midrule
...
Para encontrar observações correspondentes de um mesmo objeto em
dois {\em surveys} é necessária uma busca no espaço de coordenadas celestes.
Dada a imensa quantidade de objetos presentes tipicamente nos surveys atuais, a
verificação de todos contra todos torna-se inviável. \citet{Kunstz2000}
aplicaram uma técnica de indexação espacial chamada {\em Hierarchical
Triangular Mesh} (HTM) aos objetos do \SDSS.

Esta técnica consiste em dividir a esfera celeste numa malha de regiões
triangulares \citep{Kunstz2001}. Cada uma destas regiões triangulares do céu
...
\ref{fig:HTMBasico}). O ponto inicial da HTM é um octaedro dado pela
intersecção dos eixos $x$, $y$ e $z$ com a esfera unitária. Os oito pimeiros
nodos da HTM são os triângulos esféricos compostos por aqueles pontos,
conectados por segmentos de grandes círculos (figura \ref{fig:HTMInicio}).

% TODO: Adicionar figura - esquema básico do HTM.
\begin{figure}
	\includegraphics[width=0.5\textwidth]{figuras/test.eps}
	\caption[Todo]
	{Todo}
...
% TODO: Adicionar figura - Inicio do HTM.
\begin{figure}
	\includegraphics[width=0.5\textwidth]{figuras/test.eps}
	\caption[Todo]
	{Todo}
...
% TODO: Elaborar mais sobre HTM? Dar um exemplo de busca no SDSS?
