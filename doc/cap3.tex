...
SkyServer}. Há um clone, feito por mim\fixme, do banco de dados do {\em Data
Release} 8 do \SDSS no servidor {\em CasJobs} do \starlight \footnote{{\em
CasJobs} do \starlight: \url{http://casjobs.starlight.ufsc.br/casjobs/}}.
...
% FIXME: Deixar mais claro que fui eu que fiz.
...
A importação das tabelas de propriedades físicas do \starlight é trivial. Já a
...
tabela, o que é evitado utilizando a segunda abordagem.}.\fixme
...
dados obtidos do \SDSS. Estes dados, junto com os parâmtetros de entrada da
síntese (tabela \texttt{synthesis\_params}), foram usados como parâmetros para a
obtenção das propriedades físicas das galáxias, armazzenadas na tabela
\texttt{synthesis\_results}. Estas tabelas estão ligadas através dos campos
\texttt{SpecObjID} e \texttt{SynID}. As linhas de emissão estão armazenadas na
tabela \texttt{el\_fit}, ligada à \texttt{synthesis\_results} através do campo
\texttt{SynID}. A definição das linhas (comprimento de onda e faixas contínuo
usadas na medida) está contida na tabela \texttt{cfg\_el\_fit}. As tabelas
referentes ao \galex são explicadas na seção
\ref{sec:Crossmatch:DefAmostras:IdSDSSGalex}.

% FIXME: Tirar aux_index
\begin{figure}
	\includegraphics[width=\textwidth]{figuras/starlight-schema.eps}
...
e a identificacão da fibra utilizada para a obtenção do espectro
...
% FIXME: Procurar a SP que cria a tabela xSDSSDR7
...
objeto do catálogo do \starlight, indepente do {\em survey} utilizado. Se existe
um correspondente AIS e outro MIS para o mesmo objeto da amostra, apenas o mais
próximo é considerado. Nos campos onde os dois {surveys} se sobrepõem, há uma
maior quantidade de objetos no MIS do que no AIS, devido ao maior tempo de
...
% TODO: Justificar o abandono de FUV.
%A análise apresentada no capítulo \ref{sec:Analise} não leva em consideração a
%fotometria FUV.
...
galáxias que pertencem à amostra. O {\em redshift} deve ser superior a $0,04$
para evitar efeitos de abertura da fibra\footnote{Os espectros obtidos pelo
\SDSS foram feitos através de fibras ópticas. Cada fibra coleta a luz de uma
região de 3 segundos de arco de diâmetro. Em {\em redshifts} próximos, uma
...
\SDSS, escapando da detecção. Assim, as galáxias da amostra devem ter $0,04 < z
< 0,17$. Este foi o mesmo critério usado por \citet{CidFernandes2011}.

A figura \ref{fig:CompletezaRedshiftMag} mostra a distribuição da magnitude
absoluta $r$ das galáxias em função do {\em redshift}. Mesmo limitada em {\em
redshift}, a amostra não está completa. O corte, que vai de $-18$ em $z=0,04$
até $-22$ em $z=0,17$, ocorre devido ao limite ``fraco" de detecção do \SDSS.
Para esta amostra, as propriedades físicas e observacionais das galáxias estão
distribuídas conforme os histogramas da figura \ref{fig:HistogramasAmostra}.

\begin{figure}
	\includegraphics{figuras/completeness-volume.eps}
	\caption[Magnitude $r$ em função do {\em redshift}.]
	{Magnitude $r$ em função do {\em redshift} para a amostra \starlightUV.}
	\label{fig:CompletezaRedshiftMag}
\end{figure}

\begin{figure}
	\includegraphics{figuras/histogram-sample.eps}
	\caption[Histogramas das medidas da amostra \starlight+UV.]
	{Histogramas das medidas da amostra \starlight+UV.}
	\label{fig:HistogramasAmostra}
\end{figure}

Resumindo, a amostra \starlightUV contém os objetos do AIS com fotometria NUV
definida, limitada em {\em redshifts} entre $0,04$ e $0,17$. No total a amostra
contém $133\,244$ objetos.
